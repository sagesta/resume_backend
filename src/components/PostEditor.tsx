        ...
        <div className="w-full max-w-5xl mx-auto p-6">
            <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex gap-4">
                    ...
                    <button
                        onClick={handleSave}
                        disabled={saving}
                        className="flex items-center gap-2 bg-slate-800 text-white px-4 py-2 rounded-lg hover:bg-slate-700 disabled:opacity-50"
                    >
                    ...
                    {!isNew && (
                        <button
                            ...
                            className="flex items-center gap-2 bg-rose-800 text-white px-4 py-2 rounded-lg hover:bg-rose-700"
                        >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="space-y-6">
                    <div className="bg-white rounded-xl shadow-sm border border-slate-300 p-6 space-y-4">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-slate-900 mb-1">Slug (URL)</label>
                            <input
                                type="text"
                                value={isNew ? (formData.title.toLowerCase().replace(/[^a-z0-9]+/g, '-')) : slug}
                                readOnly
                                disabled={!isNew}
                                className="w-full p-2 border border-slate-300 rounded-md bg-slate-200 text-slate-600"
                            />
                        </div>

                        <div>
                            <label className="block text-sm font-medium text-slate-900 mb-1">Published Date</label>
                            <input
                                type="date"
                                name="published"
                        ...
                        <div className="flex items-center gap-2 pt-2">
                            ...
                            />
                            <label className="text-sm font-medium text-slate-900">Draft</label>
                        </div>
                    </div>
                </div>
            </div>
